...
class StatsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final maxDay = days.fold<int>(
      1,
      (m, d) => math.max(m, math.max(d.$2, d.$3)),
    );
    final hp = R.hPad(context);
    final wide = R.isWide(context);
    final maxW = wide ? 900.0 : R.maxContentWidth;
      ...
      children: [
        WaxHeader(
          padding: EdgeInsets.fromLTRB(
            hp + 4,
            R.isShort(context) ? 8 : 16,
            hp + 4,
            R.isShort(context) ? 16 : 26,
          ),
          child: MaxWidth(
            max: maxW,
            child: Column(
        ...
        Expanded(
          child: MaxWidth(
            max: maxW,
            child: ListView(
              padding: EdgeInsets.fromLTRB(hp, 16, hp, 120),
              children: [
                GridView.count(
                  crossAxisCount: R.statColumns(context),
                  shrinkWrap: true,
                  physics: const NeverScrollableScrollPhysics(),
                  mainAxisSpacing: 10,
                  crossAxisSpacing: 10,
                  childAspectRatio: R.isCompact(context) ? 1.15 : 1.35,
                  children: [
                    StatTile(
                ...
                // Sur écran large : graphique et mode de paiement côte à côte
                if (wide)
                  Row(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      ...
                      children: [
                        for (int i = 0; i < top.length; i++)
                          ListTile(
                            ...
                            subtitle: Text(
                              top[i].promiseDate != null && top[i].balance > 0
                                  ? Fmt.promiseLabel(top[i].promiseDate!)
  ...
  Widget _chartCard(List<(DateTime, int, int)> days, int maxDay) {
    return Card(
      child: Padding(
                        ...
                        children: [
                          Expanded(
                            child: Row(
                              crossAxisAlignment: CrossAxisAlignment.end,
                              children: [
                                _bar(d.$2, maxDay, AppColors.deepOrange),
